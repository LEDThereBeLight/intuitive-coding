...
<div style="text-align:center"><h1 style="font-size:2em;color:#333;padding-bottom: 30px">cats</h1><input/><br/><br/><button style="padding:10px;background-color:#99ddff;border-radius:2px;border:1px solid #99ddff;color:white;font-size:1.5em;cursor:pointer;margin:5px; display: inline-block;">more please!</button><br/><br/><img src="https://media.giphy.com/media/11s7Ke7jcNxCHS/giphy.gif"></div>
{: .answer}
...
If you're curious what each of these lines of CSS mean, you can look up the declarations on a <a href="http://cssreference.io/" target="_blank">CSS reference site</a>.
...
We can also add a "placeholder" to the input element, an HTML attribute that only shows up if there's no text present in the box.
...
We're going to want to update the model whenever anything is typed into the box. Let's go ahead and add a text element that will print the model to the screen so we can test it easily.
...
We need a way to update the model when the search bar's text changes, and we make updates with Messages. Let's create a new message that will trigger when the text changes. We can delete the other Messages we used to have since we're not using the buttons anymore.

{% highlight haskell %}
type Message = SearchTextChanged String
{% endhighlight %}    

We're doing something a little different here. We're parameterizing the message with a String type because we're going to need to pass the new text to our update function whenever it's called. If we didn't have this, we would know that the text changed, but not what it changed *to*. You'll see how this works in a second.

Now we need to capture the event of the user entering text into the search box. The HTML language has a builtin attribute called "[onInput](https://developer.mozilla.org/en-US/docs/Mozilla/Tech/XUL/Attribute/oninput)" that triggers on an input element whenever its text is changed. Elm accordingly has a matching Html.Attributes.Event function to match this attribute. Here's what the description says about the onInput function:

![onInput](/images/onInput.png)

The "event.target.value" section is a reference to the underlying Javascript language, but we don't need to worry about that right now. The point is that the `onInput` function captures the full text of the search box whenever the function is triggered, so we can automatically save the matching search text whenever the SearchTextChanged message is triggered. Let's add this function.
...
Now we have to change our update function to get rid of the old button messages and handle our new SearchTextChanged message. Whenever the onInput message fires, it always provides the new, complete text inside the input box as a parameter to the message. We can capture this as a variable in our case statement's pattern matching and then set the model's searchText to this new text.
...
Try refreshing the page. You should be able to type into the input box and see the model's searchText change on the screen.

### Upgrading our beginnerApp

It's time to add the functionality we need to pull in GIFs from the internet for use in our app. This is pretty complicated the first few times you see it, so don't be intimidated. *No one* understands it the first time through. As you work with it a few times, you'll start to see how everything fits together and be amazed at how simple it starts to look.
...
Instead, `Html.program`'s Record input requires four fields:
  ...
  * update, our normal `update` function with an extra twist to allow commands
  * subscriptions, a function that allows our program to "listen" to external input
  * our normal `view` function

Let's change our `main` function to use this new type of program, and then walk through what each part means.
...
Here's the type declaration for `Html.program` on the [official docs](http://package.elm-lang.org/packages/elm-lang/html/2.0.0/Html#program). Try to get some intuition about what it's telling us.
![program](images/html-program.png)

It's saying that `Html.program` takes one input, a Record, and returns a Program with some extra stuff built into it. But the record has to have a very specific structure. It needs four fields:
...
Each of these fields' values also have very specific type requirements.

* init is a Tuple of a model and a Cmd type value
* update is a function that takes a msg and a model and return a Tuple of a model and a Cmd
* subscriptions is a function that takes a model and gives back a Sub type
* view is a function that takes a model and gives back an Html value

If you're confused by this, you're not missing anything. We'll walk through each of these sections, but it really does take a couple repetitions to internalize everything. We'll start with the `init` field.
...
What we're saying is that `init` will return a Tuple made of a model with an empty topic, picture, and searchText, and a Cmd.

We could rewrite this the long way with the following equivalent code:
...
Here, `Cmd.none` is just a function in the Cmd module (imported in Elm by default) which runs *no command*. It's a way of doing nothing when something is expecting a Cmd to be passed to it.

### Subscriptions

Let's talk about the `subscriptions` field. Subscriptions let us listen for changes from anything that we don't directly control within our app. We just *subscribe* to the events we're interested in, and we get a Message whenever something happens. If we're making a clock, we might have a subscription for the time, which updates every second. Or we might have a subscription to a Twitter account that triggers whenever there's a new Tweet from that profile. In our case, we don't need any subscriptions because there's nothing external that we need to look for - all changes are handled by our view code which listens for mouse clicks and search text changes.
...
So we can declare our subscriptions function like this, to match the type signature that `Html.program` is looking for:
...
In other words, whenever the `update` function runs, it will also always run a Cmd in addition to providing an updated model. Let's get the types to match up by updating what happens when we receive a SearchTextChanged message.
...
Now we need to handle the new Message in the update function. When a user clicks the search button, we want to update the topic with the text in the search box and run a search for an image from that topic.
...
We've added a function `getRandomGif`, which we haven't defined yet. But we know the function is going to need to know the searchText in order to find a picture of that topic, so we'll provide it as an input. This new function will need to ask Giphy's API for a picture, and then we'll take the response and display it on the page.

Before we define the getRandomGif function, let's talk more about APIs.

### HTTP requests

We won't explain how HTTP requests work because there are already lots of good explanations online. [marksheet.io](http://marksheet.io/internet.html) has a short section that can get you started. We're going to make a function that will request an image from giphy.com using [Giphy's API](http://api.giphy.com/).
...
In Giphy's API documentation, there's a section that mentions a [random endpoint](https://github.com/Giphy/GiphyAPI#random-endpoint) that returns a single random GIF by topic.

![random endpoint](images/random-endpoint.png)

We can use this endpoint to fetch a random picture. What we're going to do is send a **request** to that endpoint, and the Giphy server will send us back a **response** with the information we want. Then we can **parse** the information, which just means to convert the raw text in the response into a useable format, so that we can display the image on our page.
...
The response is in [JSON](https://developer.mozilla.org/en-US/docs/Glossary/JSON) format, which is a data format modeled off of Javascript's object notation. There are lots of different data formats, but this one seems to be popular at the moment.
...
<div class="table">
  <table>
    <tr>
      ...
      <td>1XX codes are rarely used. They're just included here so you don't wonder why they're missing.</td>
    </tr>
    <tr>
      <th>200 series</th>
    ...
    <tr>
      ...
      <td>3XX codes mean the address you're requesting has changed, and you should go to a new address instead. In real life, our browser automatically follows these redirects.</td>
    </tr>
    <tr>
      <th>400 series</th>
...
Giphy's API, specifically, is set up to look for URL parameters in the form of a query string. For example, in the URL
...
The web server at Giphy is programmed to look for these names and handle the values assigned to them with the `=` sign. If it handles them successfully, it will return the information we request - otherwise, it will send back an error code.
...
There's a whole module named [Json.Decode](http://package.elm-lang.org/packages/elm-lang/core/latest/Json-Decode) made for decoding text data in the JSON format and putting it into structures that we can use in our code, and we can access it by adding an `import Json.Decode` line at the top of our file.

{% highlight haskell %}
import Json.Decode as Decode
{% endhighlight %}

With this `import` statement, we're adding in one more thing we haven't seen before. In addition to being able to expose modules to be used without any module prefix, we can also choose to use a module prefix, but with a different name than the default. In this case, we're going to use Json.Decode functions `as` Decode, so we can write `Decode.function` instead of `Json.Decode.function`. This is standard practice in Elm development.

We could spend all day talking about JSON decoders, but for now, we'll just use the simple [`Json.Decode.at` function](http://package.elm-lang.org/packages/elm-lang/core/latest/Json-Decode#at) to pull out the information.

![json decode at](images/json-decode-at.png)
...
It takes in a List of Strings that represent the location of our data, and a Decoder of any type, and gives back a new decoder. We'll then **use** this new decoder in our code on the response to pull out our `image_url`.
...
Here we're giving `Decode.at` a list of Strings to search "inside" for our data, and a String Decoder with `Decode.string`.
...
Making a request from a web server is different from the other functions we've written so far. It doesn't simply return a value, it has an *effect* in the real world. The request might succeed or it might fail, so we need code that's flexible enough to handle both alternatives. Elm makes this easy by introducing the idea of a **command**. Let's make `getRandomGif` return a Cmd, the command type in Elm, and then we'll see how it works.
